                        ...
                        <tbody>
                            @forelse ($users as $user)
                                <tr class="{{$loop->odd ? 'bg-white' : 'bg-gray-100'}}  border-b">
                                    ...
                                    <td>
                                        <div class="flex items-center">
                                            <a href="{{ route('authorizations.show', $user->id) }}">
                                                <x-primary-button class="ml-3">
                                                    {{ __('Show') }}
                                                </x-primary-button>
                                            </a>

                                            <a href="{{ route('authorizations.edit', $user->id) }}">
                                                <x-primary-button class="ml-3">
                                                    {{ __('Edit') }}
                                                </x-primary-button>
                                            </a>

                                            <form action="{{ route('authorizations.destroy', $user->id) }}" method="POST" onsubmit="return confirm('Are you sure?')"> @csrf @method('DELETE')
                                                <x-primary-button class="ml-3">
                                                    {{ __('Delete') }}
                                                </x-primary-button>
                                            </form>
